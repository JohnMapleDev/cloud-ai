...
async function resp(query, breakdown, user, device) {
    if (query.startsWith('how are you')) {
        return { text: "I'm Great! How are you?", canRespond: true };
    } else if (query.startsWith('how do you feel about')) {
        return { text: "I'm a ai, my feelings are in an early stage." };
      } else if (query.startsWith('where do you live')) {  // Move ?
          return { text: "I live In Cyberspace, where the sun never shines" };
          } else if (query.startsWith('who created you')) {  // Move ?
              return { text: "Baba Boey " };
            } else if (query.startsWith('svenska test')) {  // Move ?
                return { text: "Nu pratar jag svenska, i alla fall för tillfället" };
